import os
from pyspark.sql import Row, SparkSession, SQLContext
from entity_codes import country_names, category_names
from enum import Enum
from pyspark import SparkContext, SparkConf, SQLContext
from pyspark.sql.types import *
from pyspark.sql.functions import *
from pyspark import *
import psycopg2
import pyspark
import itertools
from datetime import datetime, timedelta
from configs import db_config,aws_config

def country_exists(r):
   if r in country_names:
       return r
   return ''

def event_exists(r):
   if r in category_names:
       return r
   return ''

country_exists = udf(country_exists,StringType())
category_exists = udf(event_exists,StringType())

def get_unprocessed_files_from_DB(connection):
    #connect to db 
    cursor = connection.cursor()
    query = "SELECT file_name FROM log_filename WHERE Isdownloaded = 1 AND Isprocessed =0 AND file_name LIKE '%export%'"
    cursor.execute(query)
    results = cursor.fetchall()
    filenames =[]
    for i in results:
        filenames.append(i[0])
    return filenames
def getTopTenEvents (t):
    country,date = t[0]
    listCategoryMentionsURL, listCategoryScoreMap = t[1]
    categoryTopEventsMap = {}
    urlCategoryToEventMap = {}
    # each event stored as (event_id, mentions, url)
    for categoryMention in listCategoryMentionsURL:
        category, numMentions, url, eventID = categoryMention
        if category not in categoryTopEventsMap:
            categoryTopEventsMap[category] = []
        # combine events with same SOURCEURL values
        if (url, category) in urlCategoryToEventMap:
            event_map = urlCategoryToEventMap[(url, category)]
            event_map['numMentions'] += numMentions
        else:
            event_map = {'eventID': eventID, 'numMentions': numMentions, 'url': url}
            categoryTopEventsMap[category].append(event_map)
            urlCategoryToEventMap[(url, category)] = event_map
        categoryTopEventsMap[category] = sorted(categoryTopEventsMap[category], key=lambda event_map: event_map['numMentions'], reverse=True)
        # if (len(categoryTopEventsMap[category]) > 10):
        #     categoryTopEventsMap[category].pop()
    return (t[0], t[1], categoryTopEventsMap)

def aggregateScores (t):
    country,date = t[0]
    listCategoryMentionsURL, listCategoryScoreMap = t[1]
    avgCategoryScoreMap = {}
    countCategoryMap = {}
    for categoryScoreMap in listCategoryScoreMap:
        category, scoreMap = categoryScoreMap
        if category not in countCategoryMap:
            countCategoryMap[category] = 0
        countCategoryMap[category] += 1
        if category not in avgCategoryScoreMap:
            avgCategoryScoreMap[category] = {}
            avgCategoryScoreMap[category]['gs'] = 0
            avgCategoryScoreMap[category]['tone'] = 0
        # calculate average of GoldsteinScale
        if scoreMap['gs']:
            avgCategoryScoreMap[category]['gs'] = ( \
                avgCategoryScoreMap[category]['gs'] * (countCategoryMap[category] - 1) + \
                scoreMap['gs'])/countCategoryMap[category]
        # calculate average of Tone
        if scoreMap['tone']:
            avgCategoryScoreMap[category]['tone'] = ( \
                avgCategoryScoreMap[category]['tone'] * (countCategoryMap[category] - 1) + \
                scoreMap['tone'])/countCategoryMap[category]
    return (t[0], t[1], t[2], avgCategoryScoreMap)

def prepareToInsert (t):
    return (t[0][0], t[0][1], t[3], t[2])

def send_event_to_SQL(rdd,connection,table_name,column_name):
    #connect to db 
    cursor = connection.cursor()
    for line in rdd.collect():
        # query = 'INSERT INTO {} ({}) VALUES (%s, %s, %s, %s)'.format(table_name,column_name)
        #         data = (line[0], line[1], line[2], line[3])
        #         cursor.execute(query, data)
        #         connection.commit()
        # print(line)
        # print(line.rn, line.cooccurence, line[2], line.weight)
                #insert the each line into the  table
                # gkg_today = spark.read.parquet("s3a://gdelt-streaming/{}*".format(today))
             
        # query = 'INSERT INTO public.cooccurences (rn,cooccurence,counts,weight) VALUES (%s, %s, %s, %s)'
        # count_column = len(column_name.split(","))
       
        query = 'INSERT INTO {} ({}) VALUES (%s, %s, %s, %s,%s,%s,%s,%s)'.format(table_name,column_name)
        country = line[0]
        year = line[1]
        stuff = {}
        for key, values in line[2].items():
            if key not in stuff:
                stuff[key] = {}
            for k2, v2 in values.items():
                stuff[key][k2] = v2

        for key, values in line[3].items():
            if key not in stuff:
                stuff[key] = {}
            for k2, v2 in values[0].items():
                stuff[key][k2] = v2                

        for key, values in stuff.items():
            # print(key)
            # print(values['gs'])
            # print(values['tone'])
            # print(values['numMentions'])
            # print(values['url'])
            # exit()
            data = (country, year, key, values['numMentions'],values['url'],values['eventID'],values['gs'],values['tone'])
            cursor.execute(query, data)
            connection.commit()
def update_log_files_SQL(connection,file_name):
    #connect to db 
    cursor = connection.cursor()
    query = "update log_filename set Isprocessed = 1 where file_name = %s)"
    cursor.execute(query, file_name)
    connection.commit()
    # sql = """ UPDATE vendors
    #             SET vendor_name = %s
    #             WHERE vendor_id = %s"""
if __name__ == '__main__':
    # Set configurations
    AWS_ACCESS_KEY = aws_config['AWS_ACCESS_KEY']
    AWS_SECRET_ACCESS_KEY = aws_config['AWS_SECRET_ACCESS_KEY']
    connection = psycopg2.connect(host = db_config['host'], database = db_config['database'], user = db_config['username'], password = db_config['password'])
   
    sc = SparkContext()
    sqlContext = SQLContext(sc)

    APP_NAME = "Global Knowledge Graph"
    conf = SparkConf().setAppName(APP_NAME)
    # conf = conf.setMaster("local[*]")
    # conf = conf.setMaster("spark://ec2-52-43-160-56.us-west-2.compute.amazonaws.com:7077")
    spark = SparkSession.builder.appName("APP_NAME") \
        .config("spark.executor.memory", "1gb") \
        .getOrCreate()

    # Set HDFS configurations
    sc=spark.sparkContext
    hadoop_conf=sc._jsc.hadoopConfiguration()
    # hadoop_conf.set("fs.s3n.impl", "org.apache.hadoop.fs.s3native.NativeS3FileSystem")
    hadoop_conf.set("fs.s3n.awsAccessKeyId", AWS_ACCESS_KEY)
    hadoop_conf.set("fs.s3n.awsSecretAccessKey", AWS_SECRET_ACCESS_KEY)
    sqlContext.setConf("spark.sql.parquet.compression.code","gzip")
    file_names = get_unprocessed_files_from_DB(connection)
    for file_name in file_names:
        event_today = spark.read.parquet("s3a://gdelt-streaming-events/{}".format(file_name))
        event_today.createOrReplaceTempView("event_today")
        # log.warn("CREATING TODAY'S MASTER DATAFRAME")

        master_event_today = spark.sql("SELECT CAST(SQLDATE AS INTEGER) as date, \
                                                CAST(MonthYear AS INTEGER) as month,\
                                                CAST(Year AS INTEGER) as year,\
                                                ActionGeo_CountryCode as country, \
                                                CASE WHEN Actor1Type1Code = '' AND Actor1Type2Code <> '' THEN Actor1Type2Code \
                                                ELSE Actor1Type1Code END AS actor_type, \
                                                CAST(GoldsteinScale AS float) as goldstein_scale, \
                                                CAST(NumMentions AS INTEGER) as num_mentions, \
                                                CAST(AvgTone AS float) as tone, \
                                                SOURCEURL as mention_source, \
                                                GLOBALEVENTID as event_id \
                                            FROM event_today ")
        # WHERE GoldsteinScale <> '' AND GoldsteinScale IS NOT NULL \
        #                                             AND SOURCEURL <> '' AND SOURCEURL IS NOT NULL
        # print(master_event_today.show(20, False))

        # event_today.take(5)
        master_event_today =  master_event_today \
                .withColumn("country",country_exists(col("country"))) \
                .filter("country != ''") \
                .filter("actor_type != ''") \
                .filter("mention_source != ''") 
                # .filter("goldstein_scale != ''")
                # .withColumn("actor_type",category_exists(col("actor_type"))) \     
        # print(master_event_today.show(20, False))
        # master_event_today.registerTempTable("filteredDF") #master_event_today
        # print(master_event_today.take(5))
        # (a["country"], a[time_period_column]),
        #             (
        #                 [(a["actor_type"], a["num_mentions"], a["mention_source"], a["event_id"])], # [("GOV", 30, "http://bbc/article123")]   [("CVL", 50, "http://cnn/article456")]
        #                 [(a["actor_type"], {"gs": a["goldstein_scale"], "tone": a["tone"]})] 
        #             ) 
        class TimePeriod(Enum):
            YEAR = 1
            MONTH = 2
            DAY = 3
        def processDataForTimePeriod(timePeriod):
            if timePeriod == TimePeriod.YEAR:
                time_period_column = "year"
                table_name = "yearly_test"
            elif timePeriod == TimePeriod.MONTH:
                time_period_column = "month"
                table_name = "monthly_test"
            elif timePeriod == TimePeriod.DAY:
                time_period_column = "date"
                table_name = "daily_test"
            else:
                return
            timePeriodCassandraRDD = master_event_today.rdd \
                    .map(lambda a: ( \
                        (a["country"], a[time_period_column]),
                        (
                            [(a["actor_type"], a["num_mentions"], a["mention_source"], a["event_id"])], # [("GOV", 30, "http://bbc/article123")]   [("CVL", 50, "http://cnn/article456")]
                            [(a["actor_type"], {"gs": a["goldstein_scale"], "tone": a["tone"]})] \
                        ) \
                    )) \
                    .reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1])) \
                    .map(getTopTenEvents) \
                    .map(aggregateScores) \
                    .map(prepareToInsert)

            # timePeriodCassandraRDD.saveToCassandra("hawkeye", table_name)
            print(timePeriodCassandraRDD.take(20)) 
            # timePeriodCassandraRDD.saveAsTextFile("s3://insightde/historial/result/timePeriodCassandraRDD2.csv")
            # timePeriodCassandraRDD_df = sqlContext.createDataFrame(timePeriodCassandraRDD)
            table_name ='public.category_events'
            column_name ='country,cyear,actor_type,numMentions,mention_source,event_id,gs,tone'
            send_event_to_SQL(timePeriodCassandraRDD,connection,table_name,column_name)
            # # timePeriodCassandraRDD_df = timePeriodCassandraRDD.map(row).toDF()
            # print(timePeriodCassandraRDD_df.show(20, False))
            # df = sqlContext.createDataFrame(rdd, schema)
            #print(df2.take(10)) RDD
            # df.show() is only for spark DataFrame
            # timePeriodCassandraRDD.write.save("s3://insightde/historial/result/timePeriodCassandraRDD.csv", format='csv', header=True)
        # processDataForTimePeriod(TimePeriod.YEAR)
        # processDataForTimePeriod(TimePeriod.MONTH)
    processDataForTimePeriod(TimePeriod.DAY)
